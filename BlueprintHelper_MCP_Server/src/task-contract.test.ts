...
describe('TaskSpec/TaskPlan protocol contract', () => {
  it('publishes one versioned contract for the first GraphWrite Append slice', () => {
  ...
  it('matches the canonical GraphWrite Append fixtures', () => {
    ...
    assert.equal(getPath(graphWriteAppendTaskSpecFixture, 'behavior.graph_strategy'), TASK_PROTOCOL_CONTRACT_V1.supported_first_slice.graph_strategy);
    ...
    assert.equal(getPath(graphWriteAppendExpectedTaskPlanFixture, 'steps.0.capability'), 'graph_write');
    assert.equal(getPath(graphWriteAppendExpectedTaskPlanFixture, 'steps.0.write.strategy'), 'owned_graph_edit');
