...
public:
	...
	static bool IsUnlinkedDataInputPin(const UEdGraphPin* Pin)
	{
		return Pin
	...
	static bool TryGetPinDefaultExportValue(const UEdGraphPin* Pin, FString& OutValue, FString& OutSource)
	{
		...
		if (!Pin->DefaultValue.IsEmpty())
		{
			...
			OutSource = TEXT("default_value");
			return true;
		}

		if (!Pin->AutogeneratedDefaultValue.IsEmpty())
		{
			OutValue = Pin->AutogeneratedDefaultValue;
			OutSource = TEXT("autogenerated_default_value");
			return true;
		}

		if (Pin->DefaultObject)
		{
			OutValue = Pin->DefaultObject->GetPathName();
			OutSource = TEXT("default_object");
			return true;
		}
		...
		if (!Pin->DefaultTextValue.IsEmpty())
		{
			...
			OutSource = TEXT("default_text_value");
			return true;
		}
	...
	TArray<TSharedPtr<FJsonValue>>& OutLinks)
{
	...
	for (int32 NodeIndex = 0; NodeIndex < Graph->Nodes.Num(); ++NodeIndex)
	{
		...
		TSharedPtr<FJsonObject> InputDefaultsObj = MakeShared<FJsonObject>();
		bool bHasInputDefaults = false;
		for (UEdGraphPin* Pin : Node->Pins)
		{
			if (!FBlueprintTextConverterLocalUtils::IsUnlinkedDataInputPin(Pin))
			{
				continue;
			}
			...
			if (FBlueprintTextConverterLocalUtils::TryGetPinDefaultExportValue(Pin, DefaultValue, DefaultSource))
			{
				const FString PinName = Pin->PinName.ToString();
				InputsObj->SetStringField(PinName, DefaultValue);

				TSharedPtr<FJsonObject> DefaultObj = MakeShared<FJsonObject>();
				DefaultObj->SetStringField(TEXT("value"), DefaultValue);
				DefaultObj->SetStringField(TEXT("source"), DefaultSource);
				DefaultObj->SetBoolField(TEXT("connected"), false);
				DefaultObj->SetObjectField(TEXT("pin_type"), PinTypeToJson(Pin->PinType));
				InputDefaultsObj->SetObjectField(PinName, DefaultObj);
