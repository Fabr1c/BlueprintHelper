...
2. 新增 `call.defaults.json`，作为 `call` pattern 的内置空基线；插件内置文件不再记录函数专属 alias、pin alias 或默认值。
...
8. `BuildCallFunctionFragment` 会应用 `call` pattern 的通用 function alias、pin alias 和默认值。
9. Qualified call 的 alias 处理只作用于函数段并保留对象段；具体函数 alias 应由项目级覆盖提供，不能写入插件内置默认文件。
...
2026-05-17 同步：已移除插件内置 `call.defaults.json` 中的具体函数示例绑定，避免内置文件携带函数专属硬编码。
