...
- 2026-05-09：用户侧编译和 Automation 状态更新。
  ...
  - ReviewPanel 后续手动 smoke 已经产生 pending visible content；早期 `sourceGraph="<none>" previewGraph="EdGraph_0" previewNodes=0` 只代表当时空 pending 状态，不再是当前阻塞。当前剩余是 live Editor 现场确认：selected-row Accept / Reject、asset-root Reject cascade、Graph diff block drawing、Debug export 边界。
