...
| `anim_notify_event` / animation notify event entry | Animation Blueprint / Animation tooling | 当前 GraphWrite 范围限定普通 Blueprint；动画蓝图事件入口不计入 GraphWrite capability gap。 |

按上述过滤后，真正仍需追踪的 GraphWrite gap 为：

| Remaining GraphWrite gap | Current status |
|---|---|
| real evidence / BuildEvidence defects | PENDING：`asset_action` 缺 Agent-facing positive TaskSpec/readback；`timer_delegate_node`、`latent_or_async_node` 已决定保留为 GraphWrite Generic schedule success path，修复执行计划为 `BlueprintHelper_GraphWrite_GenericScheduleSuccessPathPlan_20260525_CN.md`；`asset_action` Review policy 已收窄为 graph-level `graph_block`。 |
...
| `timer_delegate_node` | schedule operation and `context_evidence` pass through to C++ resolution request. | PLAN READY：执行 `BlueprintHelper_GraphWrite_GenericScheduleSuccessPathPlan_20260525_CN.md`，通过 projected ActionDatabase schedule spawner evidence + BlueprintSignature handler evidence 成功；GraphWrite 不创建 handler/signature。 |
| `latent_or_async_node` | schedule operation, `graph_latent_allowed`, and `context_evidence` can pass through. | PLAN READY：执行 `BlueprintHelper_GraphWrite_GenericScheduleSuccessPathPlan_20260525_CN.md`，通过 projected latent/async spawner evidence + `graph_latent_allowed=true` 成功；GraphWrite 不创建 handler/signature。 |
