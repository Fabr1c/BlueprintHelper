# BlueprintHelper Active Plan Index

Date: 2026-05-20

This folder contains only active or living planning documents. Completed v0.5.0-v0.5.3 records are archived under `../v0.5.4/ArchivedReference/CompletedDevelopDocs_20260520/`; completed v0.4.4-era records are archived under `../v0.4.4/ArchivedReference/CompletedDevelopDocs_20260519/`; older setup, smoke, AgentGuide, Review/Debug v1, and legacy records remain under their original version archive folders.

## Current Status And Verification

- `BlueprintHelper_UnmetExpectations_Consolidated_20260514_CN.md`
- `BlueprintHelper_ReviewPanelV2_ArchitecturePlan_20260519_CN.md`

## Active Implementation Plans

- `BlueprintHelper_ReviewPanelV2_ArchitecturePlan_20260519_CN.md`
- `BlueprintHelper_ReadContext_PinDefaults_Fix_20260518_CN.md`

## Active Design And Architecture

- `BlueprintHelper_Development_Rules_20260520_CN.md`

## Living Tips

- `BlueprintHelper_CLI_Tips_20260514_CN.md`

## Archive

- `../v0.5.4/ArchivedReference/CompletedDevelopDocs_20260520/`
- `../v0.4.4/ArchivedReference/CompletedDevelopDocs_20260519/`
- `../v0.4.3/ArchivedReference/RetiredReviewDebugDocs_20260518/`
- `../v0.4.3/ArchivedReference/RetiredAgentDocs_20260517/`
- `../v0.4.3/ArchivedReference/RetiredSetupDocs_20260517/`
- `../v0.4.1/ArchivedReference/RetiredPlanDocs_20260517/`
- `../v0.4.1/ArchivedReference/RetiredSetupDocs_20260517/`
- `../v0.3.8/`
