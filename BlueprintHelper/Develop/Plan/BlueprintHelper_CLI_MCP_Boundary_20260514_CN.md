...
当前边界修正为：BlueprintHelper 普通 Agent-facing 资产读写入口仍然是 CLI；MCP 只保留编辑器生命周期 companion 能力，用于 `blueprint_open_editor` 和 `blueprint_close_editor`。

原因：在 Codex 沙盒内，用普通 shell/CLI 启动的 UnrealEditor 子进程可能随工具调用结束被宿主清理；全局 MCP 生命周期工具能作为长驻 companion 管住编辑器启动/关闭。这个边界不改变普通资产读写主线，TaskSpec、DebugBundle、读上下文、写任务执行仍全部走 CLI。
...
当前稳定策略是：Codex 工作流使用全局 MCP 启动/关闭编辑器；编辑器就绪后，普通 BlueprintHelper 操作仍使用 CLI。
...
4. 不向 Agent 暴露或恢复旧 MCP 普通工具面。
5. 如果 MCP 生命周期工具不可用，再明确报告环境限制，并让用户在外部 PowerShell 启动/关闭编辑器。
