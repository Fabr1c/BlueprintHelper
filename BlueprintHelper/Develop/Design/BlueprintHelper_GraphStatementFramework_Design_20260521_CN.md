...
- Every GenericOps operation declares exactly one runtime owner. Function-backed container/convert/create/schedule operations route through `FunctionActionCluster`; control, StandardMacros, struct/select, `asset_action`, and generic schedule node operations route through `GenericAssetStructControlActionCluster` only when their focused evidence is present.
