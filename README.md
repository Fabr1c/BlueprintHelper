...
双击 `update.cmd` 可以检查 GitHub 最新 Release，并在发现远端版本更新时提示确认后更新。更新器以 GitHub Release tag 为准，例如 `v0.5.4`、`v0.6.4`。同版本补丁可以发布为 `v0.6.4-fix`、`v0.6.4-hotfix1` 这类带 `-` 后缀的 tag；包内 manifest 版本仍可保持 `0.6.4`，updater 会优先应用该补丁 release，并在 `%LOCALAPPDATA%\BlueprintHelper\UpdateState\` 与插件根目录 `.blueprinthelper\update-state.json` 记录已应用 tag，避免重复提示同一个补丁。
...
Double-click `update.cmd` to check the latest GitHub Release and update only after confirmation. The updater compares versions by GitHub Release tags such as `v0.5.4` and `v0.6.4`. Same-version patch releases can use tags such as `v0.6.4-fix` or `v0.6.4-hotfix1`; package manifests may remain at `0.6.4`, and the updater will prioritize applying that patch tag. Applied tags are recorded under `%LOCALAPPDATA%\BlueprintHelper\UpdateState\` and the plugin-root `.blueprinthelper\update-state.json` fallback so the same patch release is not offered repeatedly.
