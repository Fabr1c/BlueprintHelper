...
UK2Node* FAssignDelegateNodeHandler::Spawn(UEdGraph* TargetGraph, const FParsedNode& NodeData, FString& OutError) const
{
	...
	{
		OutError = TEXT("AssignDelegate 节点生成失败：委托属性名为空，请在 delegate.property_name 中指定。");
		return nullptr;
	}
	...
	if (!DelegateProperty)
	{
		OutError = FString::Printf(TEXT("AssignDelegate 节点生成失败：未在蓝图中找到事件分发器 '%s'。"), *DelegateName);
		return nullptr;
	}
	...
	// 不调用 PostPlacedNewNode()：该方法会自动创建伴生 CustomEvent 子节点并尝试连线，
