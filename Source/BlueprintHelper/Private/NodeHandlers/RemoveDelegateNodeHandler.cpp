...
bool FRemoveDelegateNodeHandler::CanHandle(EParsedBlueprintNodeType NodeType) const
{
	return NodeType == EParsedBlueprintNodeType::RemoveDelegate;
}

UK2Node* FRemoveDelegateNodeHandler::Spawn(UEdGraph* TargetGraph, const FParsedNode& NodeData, FString& OutError) const
{
	if (!TargetGraph)
	{
		OutError = TEXT("RemoveDelegate 节点生成失败：目标图表无效。");
		return nullptr;
	}
	...
	{
		OutError = TEXT("RemoveDelegate 节点生成失败：委托属性名为空，请在 delegate.property_name 中指定。");
		return nullptr;
	}
	...
	{
		OutError = TEXT("RemoveDelegate 节点生成失败：无法找到蓝图或骨架类。");
		return nullptr;
	}
	...
	if (!DelegateProperty)
	{
		OutError = FString::Printf(TEXT("RemoveDelegate 节点生成失败：未在蓝图中找到事件分发器 '%s'。"), *DelegateName);
		return nullptr;
	}

	UK2Node_RemoveDelegate* DelegateNode = NewObject<UK2Node_RemoveDelegate>(TargetGraph);
