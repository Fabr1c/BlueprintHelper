	...
	if (OutTarget.bAlreadyUserOwned)
	{
		if (Request.AlreadyUserOwnedPolicy == EBlueprintHelperAlreadyUserOwnedPolicy::Error)
		{
			...
			Result.Conflicts.Add({TEXT("already_user_owned"),
				FString::Printf(TEXT("Block %s 已经。user-owned。"), *OutTarget.BlockId),
				OutTarget.BlockId, TEXT("ownership")});
		}
	}
	...
	const FConvertRequest& Request, FResolvedBlock& OutTarget, FString& OutError) const
{
	...
	{
		OutError = TEXT("无效请求：需。block_id 。graph_id+block_ref。");
		return false;
	}
	...
	const FConvertRequest& Request) const
{
	...
	const FString TxId = Request.TransactionId.IsEmpty()
	...
	FBlueprintHelperTransactionSummary Transaction;
	Transaction.TransactionId = TxId;
	Transaction.Status = EBlueprintHelperTransactionStatus::Applied;
	Transaction.AffectedAssets.Add(Request.AssetPath);
	Transaction.BlockIds.Add(Target.BlockId);
	Transaction.bRollbackAvailable = true;
	FBlueprintHelperOwnershipSummary OwnershipSummary;
	OwnershipSummary.AffectedOwnedNodesCount = Target.BlockNodes.Num();
	OwnershipSummary.bMetadataRemoved = true;
	OwnershipSummary.bNodeCommentsRemoved = true;
	Transaction.OwnershipSummary = OwnershipSummary;
