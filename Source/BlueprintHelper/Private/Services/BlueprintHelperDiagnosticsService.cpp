...
FBlueprintHelperDiagnosticsData FBlueprintHelperDiagnosticsService::RunRuntimeDiagnostics() const
{
	...
	// UE Editor 是否运行（此代码在 UE Editor 中执行，因此必然运行）
	Report.AddInfo(TEXT("ue_editor.running"));

	// Bridge 是否可达（Service 在模块启动时创建，说明 Bridge 已完成初始化）
	...
	// Risk Command
	// 默认禁用，需要 BLUEPRINTHELPER_ENABLE_HIGH_RISK_COMMANDS=1
