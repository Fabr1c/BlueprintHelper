...
`blueprint_open_editor` / `blueprint_close_editor` 只用于显式启动或关闭目标 Editor 的 lifecycle preflight。Agent 工作流使用全局 MCP 生命周期工具；CLI lifecycle helper 只作为兼容/手动 fallback。
