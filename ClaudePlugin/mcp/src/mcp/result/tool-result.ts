/**
 * BlueprintHelper MCP �?Tool Result Base 标准�?
 *
 * �?0 簇：统一所�?MCP 工具返回体的基础字段�?
 * �?Bridge 原始响应映射�?Agent 可见的精简 Tool Result Base�?
 *
 * 约束�?
 * - 不返�?tool / command / request_id / diagnostics / next / ownership 顶层字段 / safety_profile�?
 * - content[0].text 只放短摘要�?
 * - structuredContent 放完�?ToolResultBase JSON�?
 */
...
/**
 * 将原�?BridgeResponse 标准化为 ToolResultBase�?
 * 用于 MCP 工具 handler 中替�?toToolResult()�?
 *
 * @param resp - Bridge 原始响应
 * @param operation - 公共操作名（�?append_blueprint_graph�?
 * @param overrides - 可选覆盖字段（�?target, transaction, validation�?
 */
...
/**
 * �?ToolResultBase 转换�?MCP tool result 格式�?
 * - content[0].text: 短摘�?
