...
- deciding whether to dispatch `blueprint-explorer`, `sourcecode-explorer`, `sourcecode-worker`, or `task-worker`;
...
- Dispatch `sourcecode-worker` for bounded source implementation when this gate requires C++, DataAsset, config, or interface work before Blueprint wiring.
...
- Dispatch `sourcecode-worker` only after the architecture gate requires source edits and the write scope plus verification commands are bounded.
