...
- Treat `AgentFaceService` as the sibling runtime package that owns `task-core`, `cli`, and deprecated MCP compatibility code.
...
- Do not register or prefer the MCP endpoint for normal work. The supported Agent-facing entry remains the BlueprintHelper CLI.
