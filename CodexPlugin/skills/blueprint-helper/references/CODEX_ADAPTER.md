...
- Prefer CLI for ordinary TaskSpec/ReadSpec/debug-summary work. Use global MCP only for editor open/close/modal-dismiss lifecycle. Deprecated MCP ordinary tools are not fallback paths.
- Only the Main Agent may use MCP lifecycle tools. `blueprint-explorer`, `sourcecode-explorer`, `sourcecode-worker`, and `task-worker` must not call any `mcp__blueprint_helper__*` tool.
