...
| Read one simple function body as LogicFlow; may degrade to `logic_json` when link classification is unknown | `read_context_function_logic_flow_template.json` | `blueprinthelper_read_context` |
| Read one simple event body as LogicFlow; may degrade to `logic_json` when link classification is unknown | `read_context_event_logic_flow_template.json` | `blueprinthelper_read_context` |
| Read one simple custom event body as LogicFlow; may degrade to `logic_json` when link classification is unknown | `read_context_custom_event_logic_flow_template.json` | `blueprinthelper_read_context` |
