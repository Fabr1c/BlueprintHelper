...
`blueprint_open_editor` / `blueprint_close_editor` 只用于显式启动或关闭目标 Editor 的 lifecycle preflight。`blueprint_developer_exec_console_command` 只用于 BlueprintHelper 本地开发/测试编排。普通 Agent 工作流使用 CLI；废弃 MCP 普通工具不作为 fallback，不新增测试，也不运行旧测试。
