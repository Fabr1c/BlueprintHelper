...
are the source-control pre-write path for P4/Perforce and other UE source-control
providers. Use status or checkout after a successful preview and before execute
when source control is enabled, when target assets may be read-only, or when
MCP close/save reports `checkout_required`. Stop on `checked_out_by_other`,
`source_control_conflicted`, `source_control_unavailable`, `checkout_failed`, or
`not_editable`, and report the returned `agent_message` / `recommended_action`
instead of editing.
