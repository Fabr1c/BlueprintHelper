...
describe('TaskSpec GraphWrite Append compiler', () => {
  ...
  it('rejects GraphWrite user-node mutation until a non-owned anchor contract exists', () => {
    ...
    assert.throws(
      () => compileTaskSpecToTaskPlan(spec),
      (error: unknown) => {
        assert.ok(error instanceof TaskSpecCompileError);
        assert.equal(error.code, 'unsupported_scope_policy');
        assert.equal(error.issues[0]?.path, 'scope_policy.allow_modify_user_nodes');
        assert.deepEqual(error.issues[0]?.suggested_patch, {
          op: 'replace',
