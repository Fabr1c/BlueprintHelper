...
from blueprinthelper_task.runtime.orchestrator import TaskSpecCompileError, compile_task_spec
...
class P1TaskCompilerTests(unittest.TestCase):
    ...
    def test_compiles_data_asset_with_blueprint_class_path(self):
        result = compile_task_spec(make_base_spec(
            "create_asset",
            {
                "asset_strategy": "ensure_asset",
        ...
        op = result["task_plan"]["steps"][0]["write"]["ops"][0]
        self.assertEqual(op["asset_type"], "data_asset")
        self.assertEqual(op["data_asset_class"], "/Game/BlueprintHelper/Smoke/BP_BHSmokeDataAssetClass")
        self.assertEqual(op["collision"], "reuse_if_exists")

    def test_rejects_data_asset_without_data_asset_class(self):
        spec = make_base_spec(
            "create_asset",
            {
                "asset_strategy": "ensure_asset",
        ...
        self.assertEqual(ctx.exception.code, "missing_data_asset_class")
        self.assertEqual(ctx.exception.issues[0]["path"], "behavior.asset.data_asset_class")
