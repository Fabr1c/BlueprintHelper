...
def _compile_asset_factory_task_plan(task_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if asset_type == "data_asset":
        data_asset_class = _optional_string_value(asset, "data_asset_class", "behavior.asset.data_asset_class")
        if not data_asset_class:
            _raise(
                "missing_data_asset_class",
                "asset_type=data_asset requires behavior.asset.data_asset_class.",
                "behavior.asset.data_asset_class",
                "Use a concrete UDataAsset subclass. In a new project, create a PrimaryDataAsset Blueprint class first and pass its asset path or generated class path.",
            )
        op["data_asset_class"] = data_asset_class
    else:
...
def _normalize_asset_factory_type_and_parent(asset: Dict[str, Any]) -> Tuple[str, Optional[str]]:
    ...
    if asset_type_key == "actor":
        return "blueprint_class", parent_class or "Actor"
    if asset_type_key in {"data_asset", "dataasset"}:
        return "data_asset", parent_class
