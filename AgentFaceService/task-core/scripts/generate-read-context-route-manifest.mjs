  ...
  for (const route of routes) {
    for (const field of [
      ...
      if (typeof route[field] !== 'string' || route[field].length === 0) {
        throw new Error(`ReadContext route ${route.route_id ?? '<unknown>'} has invalid ${field}.`);
      }
    }
    if (ids.has(route.route_id)) {
      throw new Error(`Duplicate ReadContext route_id: ${route.route_id}`);
    }
    ids.add(route.route_id);
    if (!['active', 'reserved'].includes(route.status)) {
      throw new Error(`ReadContext route ${route.route_id} has invalid status: ${route.status}`);
    }
    for (const arrayField of ['required_target_fields', 'supported_asset_types', 'supported_formats']) {
      if (!Array.isArray(route[arrayField]) || route[arrayField].some((entry) => typeof entry !== 'string')) {
        throw new Error(`ReadContext route ${route.route_id} has invalid ${arrayField}.`);
      }
    }
    if (route.status === 'active') {
      if (typeof route.target_type !== 'string' || route.target_type.length === 0) {
        throw new Error(`Active ReadContext route ${route.route_id} requires target_type.`);
      }
      if (typeof route.bridge_command !== 'string' || route.bridge_command.length === 0) {
        throw new Error(`Active ReadContext route ${route.route_id} requires bridge_command.`);
      }
    }
  }
}
...
function renderUeHeader(routes) {
  const rows = routes.map((route) =>
    `\t{TEXT("${escapeCppString(route.route_id)}"), TEXT("${escapeCppString(route.bridge_command ?? '')}"), TEXT("${escapeCppString(resolveCluster(route))}"), TEXT("${escapeCppString(route.domain)}"), TEXT("${escapeCppString(route.read_cluster)}"), TEXT("${escapeCppString(route.target_kind)}"), TEXT("${escapeCppString(route.view_template)}"), TEXT("${escapeCppString(route.status)}")}`);
  return `// <auto-generated>\n`
