...
function validateDescriptors(descriptors) {
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.route_cluster !== 'UMGWidget') {
      throw new Error(`UMG widget operation ${descriptor.kind} has invalid route_cluster: ${descriptor.route_cluster}`);
    }
    if (typeof descriptor.route_agent_visible !== 'boolean') {
      throw new Error(`UMG widget operation ${descriptor.kind} has invalid route_agent_visible.`);
    }
    if (typeof descriptor.route_requires_game_thread !== 'boolean') {
      throw new Error(`UMG widget operation ${descriptor.kind} has invalid route_requires_game_thread.`);
    }
